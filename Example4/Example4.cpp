...
void drawM(BlockID block, int count, bool emptyOnly)
{
	int px, py, pz;
	...
	for (int k = 0; k <= 4; k++)
	{
		drawLine(block, px, py + k, pz, px + dx, py + k, pz + dz, emptyOnly);
	}
...
int main()
{
	...
	drawM(diamond, 5, true);					// DIAMOND 블록을 플레이어의 앞으로 10개 놓는다(비어 있는 경우에만)
}